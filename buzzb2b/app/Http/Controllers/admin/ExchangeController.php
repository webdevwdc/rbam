...
use App\User, App\Role, App\Exchange, App\ExchangeRelationship;
...
class ExchangeController extends Controller
{
    ...
    public function create(){
        $data = array();
        $data['exchangeList'] = Exchange::get()->pluck('name', 'id')->all();
      ...
      $validator = Validator::make(
	    ...
	    if ($validator->fails()){
		    ...
		    return Redirect::back()->withErrors($validator->errors())->withInput();
                ...
                if(!is_null($request->get('partner_exchange_id')))
                {
                    $tempData = [];
                    $tempData['exchange_id'] = $insert->id;
                    $tempData['partner_exchange_id'] = $request->get('partner_exchange_id');
                    $tempData['accept_barter_from'] = 1;
                    $tempData['accept_giftcard_from'] = 1;
                    $tempData['viewable_on_directory'] = 1;
                    $tempData['accept_referral_from'] = 1;

                    $exchangeRelationship[] = $tempData;

                    $tempData = [];

                    $tempData['exchange_id'] = $request->get('partner_exchange_id');
                    $tempData['partner_exchange_id'] = $insert->id;
                    $tempData['accept_barter_from'] = 1;
                    $tempData['accept_giftcard_from'] = 1;
                    $tempData['viewable_on_directory'] = 1;
                    $tempData['accept_referral_from'] = 1;
                    ...
                    // dd($exchangeRelationship);

                    ExchangeRelationship::insert($exchangeRelationship);
                }
    ...
    public function edit($id){
        ...
        $partnerExchange = ExchangeRelationship::where('exchange_id', $id)->first(['partner_exchange_id']);

        if(count($partnerExchange) > 0)
        {
            $data['partnerExchangeID'] = $partnerExchange->partner_exchange_id;
        }
        else
        {
            $data['partnerExchangeID'] = '';
        }
    ...
    public function update(Request $request,$id){
	    ...
	    if ($validator->fails()){
		    ...
		    return Redirect::back()->withErrors($validator->errors())->withInput();
                ...
                ExchangeRelationship::where('exchange_id', $id)->delete();
                ExchangeRelationship::where('partner_exchange_id', $id)->delete();

                if(!is_null($request->get('partner_exchange_id')))
                {
                    $tempData = [];
                    $tempData['exchange_id'] = $id;
                    $tempData['partner_exchange_id'] = $request->get('partner_exchange_id');
                    $tempData['accept_barter_from'] = 1;
                    $tempData['accept_giftcard_from'] = 1;
                    $tempData['viewable_on_directory'] = 1;
                    ...
                    $tempData['exchange_id'] = $request->get('partner_exchange_id');
                    $tempData['partner_exchange_id'] = $id;
                    $tempData['accept_barter_from'] = 1;
                    $tempData['accept_giftcard_from'] = 1;
                    $tempData['viewable_on_directory'] = 1;
                    $tempData['accept_referral_from'] = 1;
