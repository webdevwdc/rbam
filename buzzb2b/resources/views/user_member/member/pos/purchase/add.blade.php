						...
						<h4>Select a Member to purchase from:</h4>
							...
							<div class="form-group" id="form-group-notes">
								<div class="input-group clearfix">
									...
									<input class="form-control input-lg memo" id="notes" name="notes" type="text">
								</div>
							</div>

							<div class="form-group">
								<div class=" not-fadered">
									<input id="submit-post-purchase" class="btn-success btn-lg btn-block btn-submit-form purchase_member" value="Post Purchase" type="button">
									<span class="loader" style="display: none"><img src="{{ asset('jacopo_admin/images/loading.gif')}}" alt=""></span>
								</div> 	
							</div>
						</div>
		   ...
		   <div class="show_purchase_confirmation" style="display:none;">
		 	  <h4>Approved</h4> <span class="confirmation">Confirmation #1111</span>
				...
				<div>
					...
					<div class="col-md-6">
						 <a href="{{route('member_dashboard')}}" class="btn btn-default btn-lg btn-block" style="margin-top:30px; height: 44px;">
							Go To Dashboard
				        </a>
					</div>
			...
			<!-- check barter when i click on submit button -->
			<script type="text/javascript">
			$(document).ready(function(){
				$('.purchase_member').click(function(){
		             ...
		             if(barter_amount > 0 && member_slug >0 ){
                       $.ajax({
                       	...
                       	data:{'barter_amount':barter_amount,'member_slug':member_slug,'tip':tip,'memo':memo,'_token':$('#token').val()},
                       	beforeSend: function() {
			              $(".loader").show();
			            },
